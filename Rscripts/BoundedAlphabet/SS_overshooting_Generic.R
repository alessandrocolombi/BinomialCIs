...
exp_name = paste0("Overshooting_",name,"_",trimmed_param)
...
# ymin = min(ub_An[1,],ub_Ubb_rnorm[1,]) * 1/12 #5*1e-3
# if(ymin < 0)
#   ymin = 0
ymin = 0
...
plot(0,0,  yaxt = "n",
     xlab = "log10(M_add)", ylab = "1000 * bound",
...
axis(side = 2, at = ylabs*1e-3, 
     labels = ylabs, las = 1, 
     cex.axis = 1 )
points( x = logMadd_grid, y = ub_An[2,], 
        type = "b", 
        lwd = 3, pch = 16, lty = 1,
        col = "darkgreen" ) 
points( x = logMadd_grid, y = ub_Ubb_rnorm[2,], 
        type = "b", 
        lwd = 3, pch = 16, lty = 1,
        col = "darkorange" ) 
